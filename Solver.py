...
from CustomExceptionClass import CannotConvertToNumber, UnapplicableOperator, LackOfBrackets
from Parameters_and_utilities import is_number

def perform_current_operation(curr_token, next_token, general_sum, curr_variable, current_token_index):
...
def solve_expression(tokens):
    ...
    while curr_token_index<(len(tokens)-1):
        ...
        if next_token=="(":
            ...
            closing_bracket_ind = get_enclosing_bracket_ind(tokens[curr_token_index:],curr_token_index)
            if (closing_bracket_ind!=False):
                value_in_brackets = solve_expression(tokens[curr_token_index+2:closing_bracket_ind])
                next_token = value_in_brackets
                curr_token_index=closing_bracket_ind
            else:
                raise  LackOfBrackets(curr_token_index)

        general_sum, curr_variable = perform_current_operation(current_token, next_token, general_sum, curr_variable, curr_token_index)
        curr_token_index+=1
    general_sum += curr_variable
